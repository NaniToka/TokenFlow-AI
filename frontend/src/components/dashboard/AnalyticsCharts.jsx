...
export default function AnalyticsCharts({ stats }) {
    ...
    return (
      <Card title="Live Analytics & Token Reduction Trends" subtitle="Real-time performance charting from API session logs">
        <div className="py-16 flex flex-col items-center justify-center text-center space-y-3 border-2 border-dashed border-zinc-800 rounded-xl bg-zinc-950/40">
          ...
          <h4 className="text-base font-semibold text-zinc-300">No Analytics History Yet</h4>
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-zinc-900 border border-zinc-700 p-3 rounded-xl shadow-xl text-xs space-y-1.5">
        ...
        <div className="h-72 w-full pt-4">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="original" name="Original Tokens" fill="#f43f5e" radius={[4, 4, 0, 0]} barSize={24} />
              <Bar dataKey="compressed" name="Compressed Tokens" fill="#10b981" radius={[4, 4, 0, 0]} barSize={24} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </Card>
